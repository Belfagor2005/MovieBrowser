...
from os.path import exists, join, getsize, getmtime, isdir, normpath
from os import statvfs, remove, rename, system as os_system, popen, walk, makedirs, listdir
from re import sub, search, findall, IGNORECASE, S
from io import open
import sys
...
def threadGetPage(url=None, file=None, key=None, success=None, fail=None):
    ...
    try:
        print(
            "[MovieBrowser][threadGetPage] url=%s, file=%s, key=%s",
            url,
            file,
            key)
...
DATABASE_CACHE_HDD = "/media/hdd/moviebrowser/cache"
DATABASE_CACHE_USB = "/media/usb/moviebrowser/cache"
...
class movieBrowserMetrix(Screen):
    ...
    def showDefaultBackdrop(self):
        # config.plugins.moviebrowser.cachefolder.value + '/default_backdrop.png'
        backdrop = default_backdrop
        # config.plugins.moviebrowser.cachefolder.value + '/default_backdrop.m1v'
...
class UpdateDatabase():
    ...
    def makeDataEntry(self, count, content):
        if self.renew is False:
            ...
            try:
                if content is True:
                    self.moviecount += 1
                    data = self.namelist[count] + ':::' + self.movielist[count] + ':::' + self.datelist[count] + ':::' + self.infolist[count][0] + ':::' + self.infolist[count][1] + ':::' + self.infolist[count][2] + ':::' + self.infolist[count][3] + \
                    ...
                    self.newseries = False
                    data = self.namelist[count] + ':::' + self.movielist[count] + ':::' + self.datelist[count] + ':::' + self.infolist[count][0] + ':::' + self.infolist[count][1] + ':::' + self.infolist[count][2] + ':::' + self.infolist[count][3] + \
                        ':::' + self.infolist[count][4] + ':::' + self.infolist[count][5] + ':::' + self.infolist[count][6] + ':::' + self.plotlist[count] + ':::' + self.posterlist[count] + ':::' + self.backdroplist[count] + ':::Series:Top:::unseen:::\n'
                else:
    ...
    def sortDatabase(self):
        ...
        f.close()
        fseries = open(DATABASE_PATH + '.series', 'w')
        fseries.write(series)
        fseries.close()
        fseries = open(DATABASE_PATH + '.series', 'r')
        series = fseries.readlines()
        series.sort(key=lambda line: line.split(':::')[0])
        fseries.close()
        fseries = open(DATABASE_PATH + '.series', 'w')
        ...
        f.close()
        fmovies = open(DATABASE_PATH + '.movies', 'w')
        fmovies.write(movies)
        fmovies.close()
        fmovies = open(DATABASE_PATH + '.movies', 'r')
        lines = fmovies.readlines()
        fmovies.close()
        self.sortorder = config.plugins.moviebrowser.sortorder.value
        try:
